/**
 * MHRV over JDBC — Google Apps Script Relay
 *
 * This script acts as a bridge between MySQL queries and HTTP requests for the
 * MasterHttpRelayVPN (MHRV) protocol. Instead of receiving HTTP POST requests,
 * it processes tunnel operations encoded as MySQL queries via JDBC connections.
 *
 * FOUR modes (adapted for JDBC):
 *   1. Single relay:  JDBC query with base64 payload           → HTTP { s, h, b }
 *   2. Batch relay:   JDBC query with base64 payload           → HTTP { q: [{s,h,b}, ...] }
 *   3. Tunnel:        JDBC MHRV_TUNNEL query                    → { sid, d, eof }
 *   4. Tunnel batch:  JDBC MHRV_BATCH query                     → { r: [...] }
 *      Batch ops include TCP (`connect`, `data`) and UDP (`udp_open`, `udp_data`)
 *
 * Tunnel operations are encoded as base64 in MySQL queries:
 * - SELECT MHRV_TUNNEL('base64_json_payload') - Single operation
 * - SELECT MHRV_BATCH('base64_json_payload') - Batch operations
 *
 * CONFIGURATION REQUIRED - CHANGE THESE TO YOUR OWN VALUES!
 */

const AUTH_KEY = "CHANGE_ME_TO_A_STRONG_SECRET";
const TUNNEL_JDBC_URL = "jdbc:mysql://YOUR_MYSQL_SERVER:3306?useSSL=false&allowPublicKeyRetrieval=true";
...
function _doTunnel(req) {
  ...
  // Execute MySQL query instead of HTTP request
  var result = _executeTunnelQuery(payload);
  if (result.error) {
    return _json({ e: result.error });
  }

  return ContentService.createTextOutput(result.response)
...
function _doTunnelBatchForward(ops) {
  var result = _executeTunnelBatchQuery({ k: TUNNEL_AUTH_KEY, ops: ops });
  if (result.error) {
    return _json({ e: result.error });
  }
  return ContentService.createTextOutput(result.response)
...
// Execute a single tunnel operation via JDBC
// Returns { response: string } on success or { error: string } on failure
function _executeTunnelQuery(payload) {
  try {
    var conn = Jdbc.getConnection(TUNNEL_JDBC_URL, TUNNEL_JDBC_USER, TUNNEL_JDBC_PASSWORD);
    var payloadJson = JSON.stringify(payload);
    var payloadB64 = Utilities.base64Encode(payloadJson, Utilities.Charset.UTF_8);
    var query = "SELECT MHRV_TUNNEL('" + payloadB64 + "')";

    var stmt = conn.createStatement();
    var rs = stmt.executeQuery(query);

    if (rs.next()) {
      var resultB64 = rs.getString(1);
      var resultJson = Utilities.base64Decode(resultB64, Utilities.Charset.UTF_8);
      var result = Utilities.newBlob(resultJson).getDataAsString();
      ...
      stmt.close();
      conn.close();
      return { error: "No result from tunnel query" };
    }
  } catch (err) {
    return { error: "JDBC tunnel error: " + String(err) };
  }
}

// Execute a batch tunnel operation via JDBC
// Returns { response: string } on success or { error: string } on failure
function _executeTunnelBatchQuery(payload) {
  try {
    var conn = Jdbc.getConnection(TUNNEL_JDBC_URL, TUNNEL_JDBC_USER, TUNNEL_JDBC_PASSWORD);
    var payloadJson = JSON.stringify(payload);
    var payloadB64 = Utilities.base64Encode(payloadJson, Utilities.Charset.UTF_8);
    var query = "SELECT MHRV_BATCH('" + payloadB64 + "')";
    ...
    if (rs.next()) {
      ...
      stmt.close();
      conn.close();
      return { error: "No result from tunnel batch query" };
    }
  } catch (err) {
    return { error: "JDBC tunnel batch error: " + String(err) };
  }
}
